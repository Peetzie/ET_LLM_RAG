...
vectorstore = Chroma.from_documents(
    documents, embeddings, persist_directory=str(Path(root_dir, "chroma"))
)
...
# %%
vectorstore = Chroma(
    persist_directory=str(Path(root_dir, "chroma")), embedding_function=embeddings
)
# Define retriever
retriever = vectorstore.as_retriever(search_type="mmr", search_kwargs={"k": 2})
...
tokenizer_llm = AutoTokenizer.from_pretrained(
    "HuggingFaceH4/zephyr-7b-alpha", cache_dir=model_dir
)
model_llm = AutoModelForCausalLM.from_pretrained(
    "HuggingFaceH4/zephyr-7b-alpha", cache_dir=model_dir
).to(device)

# Assume `docs_rel` contains the relevant documents you've retrieved
# Concatenate their content to form the context
context = " ".join([doc.page_content for doc in docs_rel])
...
{query}
You have the given context to help you answer the question {context}
</s>
"""

# %%
# Encode and generate response
input_ids = tokenizer_llm.encode(prompt, return_tensors="pt").to(device)
output_ids = model_llm.generate(
    input_ids, max_length=1024, temperature=0.7, num_return_sequences=1
)

# Decode the generated ids to text
response = tokenizer_llm.decode(output_ids[0], skip_special_tokens=True)
print(response)
